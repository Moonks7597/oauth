package io.oauth2.server.client.service;
...
public class ClientService {
    ...
    public ResponseEntity<?> clientRegister(Client client) {
        ...
        String grantType = GRANT_TYPE_REFRESH_TOKEN + "," +
                GRANT_TYPE_PASSWORD + "," +
                GRANT_TYPE_CLIENT_CREDENTIALS + "," +
                GRANT_TYPE_AUTHORIZATION_CODE;
        String scope = SCOPES_READ + "," + SCOPES_WRITE;

        client.setClient(randomStringCreateUtils.getRandomString(32),
                LocalDate.now(),
                "{noop}" + randomStringCreateUtils.getRandomString(32),
                LocalDate.now(),
                AUTH_METHOD_CLIENT_SECRET_BASIC,
                grantType,
                "",
                scope,
                writeMap(clientSetting.getSettings()),
                writeMap(tokenSetting.getSettings()));
